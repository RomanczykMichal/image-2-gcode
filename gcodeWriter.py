...
"""
    ...
    def move_to_point(self, point):
        self.gcode += (
            self.commands["positioning"]
            + self.commands["x"]
            + str(point["x"] + self.X_offset)
            + self.commands["y"]
            + str(point["y"] + self.Y_offset)
            + self.new_line
        )

    def lift(self):
        self.gcode += (
            self.commands["positioning"] + self.commands["z"] + "5" + self.new_line
        )

    def lower(self):
        self.gcode += (
            self.commands["positioning"] + self.commands["z"] + "0" + self.new_line
        )

    def draw_line(self, point):
        self.gcode += (
            self.commands["linear_interpolation"]
            + self.commands["x"]
            + str(point["x"] + self.X_offset)
            + self.commands["y"]
            + str(point["y"] + self.Y_offset)
